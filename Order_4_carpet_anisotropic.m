...
N_x = 20;                   % Space discretization, with h_x = λ/N_x.
N_z = 0.1*N_x^2;            % Space discretization, with h_z = λ/N_z.

M = sqrt(2)*N_z;                        % Time discretization, with τ = f/M.
r = ( 1/M / (1/N_x + 1/N_z) )^2;        % Must be smaller than 1 if we want a stable algorithm.
a_discrete = ceil( w/2/lambda*N_x );    % Grating's discrete width.

tau = lambda/c/M; % Temporal step, τ = f/M.
h_x = lambda/N_x; % Spatial X step (every matrix element represents an area of hxh). hX = λ/N.
h_z = lambda/N_z; % Spatial Z step (every matrix element represents an area of hxh). hZ = λ/N.

n_x = ceil( X_max/h_x ); % Number of points in the x direction.
n_z = ceil( Z_max/h_z ); % Number of points in the z direction.
...
t = 0;                                      % Current time
x = double((-3*X_max:double(h_x):3*X_max)); % Matrix with the x-axis positions
z = double((0:double(h_z):Z_max));          % Matrix with the z-axis positions
        ...
        for m = 3:tstep
            for l = 3:n_x+3
                u_despues(l,m) = 2*u_ahora(l,m) - u_antes(l,m) +...
                    (c*tau)^4 /12 * laplacian_squared(u_ahora,l,m,h_x,h_z) +...
                    (c*tau)^2 * laplacian(u_ahora,l,m,h_x,h_z);
            end
        end
...
function lapl = laplacian(u,l,m,h_x,h_z)

    lapl = -5/2*u(l,m) / h_x^2 - 5/2*u(l,m) / h_z^2 + ...
           4*( u(l+1,m) + u(l-1,m) )/3 / h_x^2 + ...
           4*( u(l,m+1) + u(l,m-1) )/3 / h_z^2 + ...
           - ( u(l+2,m) + u(l-2,m) )/12 / h_x^2 + ...
           - ( u(l,m+2) + u(l,m-2) )/12 / h_z^2;
    
end

% Computes the squared laplacian of u at (l,m)
% A second order scheme is used
function lapl2 = laplacian_squared(u,l,m,h_x,h_z)
    
    lapl2 = ( u(l-2,m) - 4*u(l-1,m) + 6*u(l,m) - 4*u(l+1,m) + u(l+2,m) ) / h_x^4 + ...
            ( u(l,m-2) - 4*u(l,m-1) + 6*u(l,m) - 4*u(l,m+1) + u(l,m+2) ) / h_z^4 + ...
            2*( 4*u(l,m) + ( u(l+1,m+1) + u(l+1,m-1) + u(l-1,m+1) + u(l-1,m-1) ) - 2*( u(l+1,m) + u(l-1,m) + u(l,m+1) + u(l,m-1) ) ) / h_x^2 / h_z^2;
